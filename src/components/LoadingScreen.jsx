...
export default function LoadingScreen({ onComplete }) {
  ...
  useEffect(() => {
    const timers = [
      ...
      setTimeout(() => setTextIn(true), 3100),
  ...
  const islandScale   = phase >= 3 ? 1.15 : phase >= 2 ? 0.32 : 0.04
  const islandBlur    = phase >= 3 ? 0 : phase >= 2 ? 5 : 16
  const islandOpacity = phase >= 1 ? 1 : 0
  const tiltX         = phase >= 3 ? 0 : 10
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        transform:`scale(${islandScale}) perspective(800px) rotateX(${tiltX}deg)`,
        filter:`blur(${islandBlur}px)`,
        opacity:islandOpacity, transformOrigin:'center center',
        transition: phase >= 3
          ? 'transform 1.6s cubic-bezier(0.16,1,0.3,1), filter 1.4s ease, opacity 1s ease'
          : 'transform 2s cubic-bezier(0.16,1,0.3,1), filter 1.8s ease, opacity 1.2s ease',
      }} />

      <div style={{
        position:'absolute', inset:0, pointerEvents:'none',
        background:`radial-gradient(ellipse at center, rgba(4,4,10,${phase >= 3 ? 0.38 : 0.7}) 0%, rgba(4,4,10,${phase >= 3 ? 0.72 : 0.92}) 100%)`,
      ...
      <div style={{
        ...
        <div style={{
          ...
          <img src="https://static.wixstatic.com/media/38fefd_6918bd121bcf48d6a348508e22b4bb38~mv2.png/v1/crop/x_1482,y_1683,w_1234,h_801/fill/w_268,h_174,al_c,q_85,usm_0.66_1.00_0.01,enc_avif,quality_auto/The%20Voice%20Company_gold-03-03.png" alt="The Voice Company" style={{ width:100, objectFit:'contain' }} />
        </div>
        ...
        <div style={{
          height:1, marginBottom:14, width:textIn?260:0,
        ...
        <div style={{
          ...
          opacity:textIn?1:0,
          animation:textIn?'riseIn 0.8s 0.3s ease forwards':'none',
