...
const LOCATIONS = [
  { id: 'cave',          number: 1, title: ['Crystal','Resonance Cave'],    desc: 'Discover the power of your voice within.',        x: '14%', y: '21%' },
  { id: 'forest',        number: 2, title: ['Harmonic','Forest'],           desc: 'Breathe. Connect. Find your natural tone.',        x: '43%', y: '15%' },
  { id: 'stage',         number: 3, title: ['Golden','Amphitheatre'],       desc: 'Perform. Inspire. Be heard.',                      x: '71%', y: '20%' },
  { id: 'constellation', number: 4, title: ['Constellation','Pathway'],     desc: 'Follow the stars. Unlock your potential.',         x: '14%', y: '52%' },
  { id: 'aria',          number: 5, title: ['Celestial','Conservatory'],    desc: 'Nurture your voice. Refine your artistry.',         x: '44%', y: '59%' },
  { id: 'tower',         number: 6, title: ['Tower of','Mastery'],          desc: 'Elevate your voice. Master your craft.',            x: '71%', y: '53%' },
]
...
const GoldBtn = ({ style, children, ...props }) => (
  <button {...props} style={{
    ...
    borderRadius: 3, padding: '8px 18px', cursor: 'none',
    fontFamily: "'Cinzel', serif", fontSize: '0.48rem',
    color: 'rgba(212,175,55,0.8)', letterSpacing: '0.18em',
...
export default function App() {
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', display: 'flex', background: '#04040a', overflow: 'hidden', fontFamily: "'Raleway', sans-serif" }}>
      ...
      {/* ── Left Sidebar ──────────────────────────────────────────────────────── */}
      <div style={{
        width: 80, height: '100%', background: 'rgba(4,4,10,0.92)',
        borderRight: '1px solid rgba(212,175,55,0.18)',
        display: 'flex', flexDirection: 'column', alignItems: 'center',
        padding: '20px 0', zIndex: 30, flexShrink: 0,
      }}>
        <div style={{ marginBottom: 28, textAlign: 'center' }}>
          <div style={{ fontSize: 22, color: '#D4AF37', lineHeight: 1 }}>✦</div>
          <div style={{ fontFamily: "'Cinzel', serif", fontSize: '0.38rem', color: '#D4AF37', letterSpacing: '0.15em', marginTop: 4, lineHeight: 1.4 }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 4, width: '100%' }}>
          {NAV_ITEMS.map((item, i) => (
            <button key={i} onClick={() => handleNav(i)} style={{
              ...
              <span style={{ fontSize: 14, color: activeNav === i ? '#D4AF37' : 'rgba(248,246,240,0.45)', marginBottom: 3 }}>{item.icon}</span>
              <span style={{ fontFamily: "'Cinzel', serif", fontSize: '0.3rem', color: activeNav === i ? '#D4AF37' : 'rgba(248,246,240,0.35)', letterSpacing: '0.12em', lineHeight: 1.3, textAlign: 'center' }}>{item.label}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      {sidePanel && NAV_PANELS[sidePanel] && (
        <div style={{
          position: 'fixed', left: 80, top: 0, bottom: 0, width: 300,
          ...
          <div style={{ padding: '28px 24px 20px', borderBottom: '1px solid rgba(212,175,55,0.15)' }}>
            ...
            <div style={{ fontFamily: "'Cinzel', serif", fontSize: '0.9rem', color: '#F8F6F0', letterSpacing: '0.12em' }}>
            ...
            {NAV_PANELS[sidePanel].content.map((item, i) => (
              ...
              >
                <div style={{
                  ...
                  fontSize: 10, color: '#D4AF37', marginTop: 1,
                }}>{item.done ? '✓' : ''}</div>
                <div>
                  <div style={{ fontFamily: "'Cinzel', serif", fontSize: '0.52rem', color: item.done ? '#D4AF37' : 'rgba(248,246,240,0.6)', letterSpacing: '0.08em', marginBottom: 3 }}>{item.label}</div>
                  <div style={{ fontFamily: "'Raleway', sans-serif", fontSize: '0.42rem', color: 'rgba(248,246,240,0.4)', letterSpacing: '0.04em' }}>{item.detail}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <button onClick={() => { setSidePanel(null); setActiveNav(0) }} style={{
            ...
            fontSize: '0.42rem', letterSpacing: '0.18em', cursor: 'none',
            transition: 'all 0.2s',
          }}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden' }}>
        ...
        {/* ── Header ──────────────────────────────────────────────────────────── */}
        <div style={{
          height: 58, background: 'rgba(4,4,10,0.88)',
          borderBottom: '1px solid rgba(212,175,55,0.18)',
          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          padding: '0 28px', flexShrink: 0, zIndex: 20,
        }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 14 }}>
            <span style={{ fontFamily: "'Cinzel', serif", fontSize: 'clamp(1.2rem, 2.2vw, 1.8rem)', color: '#D4AF37', letterSpacing: '0.15em', fontWeight: 700 }}>VOICEIQ</span>
            <span style={{ fontFamily: "'Raleway', sans-serif", fontSize: '0.52rem', color: 'rgba(248,246,240,0.4)', letterSpacing: '0.25em', textTransform: 'uppercase' }}>Your AI Vocal Practice Companion</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontFamily: "'Raleway', sans-serif", fontSize: '0.52rem', color: 'rgba(248,246,240,0.5)', letterSpacing: '0.15em' }}>Welcome back,</div>
              <div style={{ fontFamily: "'Cinzel', serif", fontSize: '0.62rem', color: '#F8F6F0', letterSpacing: '0.08em' }}>Vocalist</div>
            </div>
            <div style={{ width: 38, height: 38, borderRadius: '50%', background: 'linear-gradient(135deg,#1a0f00,#3a2400)', border: '1.5px solid rgba(212,175,55,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, color: '#D4AF37' }}>♪</div>
            <div style={{ background: 'rgba(212,175,55,0.12)', border: '1px solid rgba(212,175,55,0.35)', borderRadius: 3, padding: '3px 10px', fontFamily: "'Cinzel', serif", fontSize: '0.42rem', color: '#D4AF37', letterSpacing: '0.12em' }}>⭐ LEVEL 12</div>
          </div>
        </div>
        ...
        <div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
          ...
          {LOCATIONS.map(loc => (
            <LocationMarker key={loc.id} loc={loc} hovered={hoveredLoc === loc.id}
              onEnter={() => setHoveredLoc(loc.id)} onLeave={() => setHoveredLoc(null)}
          ...
          <div style={{ position: 'absolute', right: 16, top: '50%', transform: 'translateY(-50%)', display: 'flex', flexDirection: 'column', gap: 8 }}>
            {['⋮', '✦', '▲'].map((icon, i) => (
              <div key={i} style={{ width: 34, height: 34, borderRadius: '50%', background: 'rgba(4,4,10,0.75)', border: '1px solid rgba(212,175,55,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(212,175,55,0.7)', fontSize: 13, cursor: 'none' }}>{icon}</div>
            ))}
          </div>
        </div>

        {/* ── Bottom Bar ──────────────────────────────────────────────────────── */}
        <div style={{ height: 68, background: 'rgba(4,4,10,0.9)', borderTop: '1px solid rgba(212,175,55,0.18)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 28px', flexShrink: 0, zIndex: 20 }}>

          <GoldBtn onClick={() => setInspModal(true)}>✦ DAILY INSPIRATION</GoldBtn>

          <div style={{ textAlign: 'center' }}>
            <div style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: 'italic', fontSize: 'clamp(0.7rem,1.1vw,0.9rem)', color: 'rgba(248,246,240,0.7)', letterSpacing: '0.04em' }}>Welcome to your vocal journey.</div>
            <div style={{ fontFamily: "'Cinzel', serif", fontSize: '0.52rem', color: '#D4AF37', letterSpacing: '0.22em', marginTop: 2 }}>EXPLORE · PRACTICE · EVOLVE</div>
          </div>
      ...
      {inspModal && (
        <div onClick={() => setInspModal(false)} style={{
          ...
          <div onClick={e => e.stopPropagation()} style={{
            background: 'linear-gradient(160deg, #0c0b14, #08070f)',
            border: '1px solid rgba(212,175,55,0.35)',
            borderRadius: 6, padding: '48px 56px', maxWidth: 520, width: '90%',
            textAlign: 'center', position: 'relative',
            boxShadow: '0 0 60px rgba(212,175,55,0.12)',
          }}>
            <div style={{ fontSize: 32, color: '#D4AF37', marginBottom: 20 }}>✦</div>
            <div style={{ fontFamily: "'Cinzel', serif", fontSize: '0.55rem', color: 'rgba(212,175,55,0.6)', letterSpacing: '0.3em', marginBottom: 28 }}>DAILY INSPIRATION</div>
            <div style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: 'italic', fontSize: 'clamp(1.1rem, 2vw, 1.5rem)', color: '#F8F6F0', lineHeight: 1.6, marginBottom: 24 }}>
              "{INSPIRATIONS[inspIdx].quote}"
            </div>
            <div style={{ fontFamily: "'Raleway', sans-serif", fontSize: '0.52rem', color: 'rgba(212,175,55,0.6)', letterSpacing: '0.12em' }}>{INSPIRATIONS[inspIdx].author}</div>
            <div style={{ marginTop: 36, width: '60%', height: 1, background: 'linear-gradient(90deg, transparent, rgba(212,175,55,0.3), transparent)', margin: '36px auto 0' }} />
            <GoldBtn onClick={() => setInspModal(false)} style={{ marginTop: 28 }}>CLOSE</GoldBtn>
          </div>
        </div>
      )}
      ...
      {analysisModal && (
        <div onClick={() => setAnalysisModal(false)} style={{
          position: 'fixed', inset: 0, zIndex: 300,
          background: 'rgba(4,4,10,0.85)', backdropFilter: 'blur(8px)',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          animation: 'fadeIn 0.3s ease',
          ...
          <div onClick={e => e.stopPropagation()} style={{
            background: 'linear-gradient(160deg, #0c0b14, #08070f)',
            border: '1px solid rgba(212,175,55,0.35)',
            borderRadius: 6, padding: '40px 48px', maxWidth: 560, width: '90%',
            boxShadow: '0 0 60px rgba(212,175,55,0.12)',
          }}>
            <div style={{ fontFamily: "'Cinzel', serif", fontSize: '0.55rem', color: 'rgba(212,175,55,0.6)', letterSpacing: '0.3em', marginBottom: 6 }}>VOICE ANALYSIS</div>
            <div style={{ fontFamily: "'Cinzel', serif", fontSize: 'clamp(1rem,1.8vw,1.4rem)', color: '#F8F6F0', letterSpacing: '0.1em', marginBottom: 32 }}>Your Vocal Profile</div>
            ...
            {[
              ...
              <div key={i} style={{ marginBottom: 18 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                  <span style={{ fontFamily: "'Raleway', sans-serif", fontSize: '0.5rem', color: 'rgba(248,246,240,0.7)', letterSpacing: '0.1em' }}>{stat.label}</span>
                  <span style={{ fontFamily: "'Cinzel', serif", fontSize: '0.52rem', color: stat.color }}>{stat.value}%</span>
                </div>
                <div style={{ height: 4, background: 'rgba(255,255,255,0.06)', borderRadius: 2, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${stat.value}%`, background: `linear-gradient(90deg, ${stat.color}88, ${stat.color})`, borderRadius: 2, transition: 'width 1s ease' }} />
                </div>
              </div>
            ))}
            ...
            <div style={{ marginTop: 28, padding: '16px 20px', background: 'rgba(212,175,55,0.06)', border: '1px solid rgba(212,175,55,0.15)', borderRadius: 4 }}>
              <div style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: 'italic', fontSize: '0.75rem', color: 'rgba(248,246,240,0.65)', lineHeight: 1.6 }}>
      ...
      {ModalScreen && (
        <div style={{
          ...
          <button onClick={closeModal} style={{
            position: 'absolute', top: 24, left: 32, zIndex: 210,
  ...
  return (
    <div onClick={onClick} onMouseEnter={onEnter} onMouseLeave={onLeave} style={{
      ...
      <div style={{
        ...
        background: hovered ? 'rgba(212,175,55,0.3)' : 'rgba(4,4,10,0.75)',
        border: `1.5px solid ${hovered ? '#D4AF37' : 'rgba(212,175,55,0.55)'}`,
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        boxShadow: hovered ? '0 0 20px rgba(212,175,55,0.5),0 0 40px rgba(212,175,55,0.2)' : '0 0 12px rgba(212,175,55,0.2)',
        transition: 'all 0.3s', marginBottom: 6,
      }}>
        <span style={{ fontFamily: "'Cinzel', serif", fontSize: '0.75rem', color: '#D4AF37', fontWeight: 700 }}>{loc.number}</span>
      </div>
      <div style={{
        background: hovered ? 'rgba(4,4,10,0.92)' : 'rgba(4,4,10,0.78)',
        border: `1px solid ${hovered ? 'rgba(212,175,55,0.5)' : 'rgba(212,175,55,0.22)'}`,
        borderRadius: 4, padding: '8px 12px', textAlign: 'center',
        minWidth: 110, maxWidth: 145, transition: 'all 0.3s',
        boxShadow: hovered ? '0 4px 24px rgba(0,0,0,0.6)' : '0 2px 12px rgba(0,0,0,0.4)',
      }}>
        <div style={{ fontFamily: "'Cinzel', serif", fontSize: '0.5rem', color: hovered ? '#F5E6A3' : '#D4AF37', letterSpacing: '0.08em', fontWeight: 700, lineHeight: 1.4, textTransform: 'uppercase', marginBottom: 4 }}>
          {loc.title[0]}<br />{loc.title[1]}
        </div>
        <div style={{ fontFamily: "'Raleway', sans-serif", fontSize: '0.38rem', color: 'rgba(248,246,240,0.6)', letterSpacing: '0.05em', lineHeight: 1.5 }}>{loc.desc}</div>
        {hovered && (
          <div style={{ marginTop: 8, fontFamily: "'Cinzel', serif", fontSize: '0.36rem', color: '#D4AF37', letterSpacing: '0.2em', borderTop: '1px solid rgba(212,175,55,0.25)', paddingTop: 6 }}>ENTER →</div>
        )}
      </div>
    </div>
  )
